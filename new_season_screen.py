...
class AddSeasonScreen(tk.Toplevel):
    """
    The screen to add a season for a user
    """
    ...
    def add_season(self,event=None):
        """
        Adds the season to the user
        """
        # Adds the season to the current user and saves the user data
        self._master.user.add_season(self._year_var.get(),num_of_games=self._games_var.get())
        self._master.save_user()

        # If the team box was checked, launch the Team screen
